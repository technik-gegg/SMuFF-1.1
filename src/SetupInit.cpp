...
void initAdaNeoPx() {
  ...
  uint16_t pixelCount = smuffConfig.ledsPerTools * smuffConfig.toolCount;
  cTools = new Adafruit_NeoPixel(pixelCount, NEOPIXEL_TOOL_PIN, COLOR_ORDER_TOOL);
  cTools->setBrightness(BRIGHTNESS_TOOL);
  __debugS(D, PSTR("\tinitAdaNeoPx: Tools initialized, %d LED's per tool"), smuffConfig.ledsPerTools);
#endif
}
