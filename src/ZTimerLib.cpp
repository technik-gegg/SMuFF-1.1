...
static void (*__timer8Hook)(void) = NULL;
  ...
  void ZTimer::setupTimer(IsrTimer timer, unsigned int prescaler, uint64_t compare) {
    _timer = timer;
...
void ZTimer::setNextInterruptInterval(timerInterval_t interval) {
  stopTimer();
...
timerInterval_t ZTimer::getOverflow() {
  switch (_timer) {
    default: return 0;
    #ifdef __AVR__
...
void ZTimer::setOverflow(timerInterval_t value) {
  switch (_timer) {
    default: break;
    #ifdef __AVR__
...
void ZTimer::setCompare(timerInterval_t value) {
  switch (_timer) {
    default: break;
    #ifdef __AVR__
      // Was not defined, then?
    #elif defined(__STM32F1__)
...
void ZTimer::setCounter(timerInterval_t value) {
  switch (_timer) {
    default: break;
    #ifdef __AVR__
