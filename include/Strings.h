...
const char P_ConfigWriteFail[] PROGMEM      = { "Config save failed!\nPlease check SD-Card." };
...
const char P_AccelSpeed[] PROGMEM     = { "X (Selector):\t%ld mm/s, Delay: %d\nY (Revolver):\t%ld mm/s, Delay: %d\nZ (Feeder):\t%ld mm/s%s, Delay: %d\n" };
...
const char P_TMC_Setup24[] PROGMEM  = { "PWM Offset      " };
...
const char P_ActionTool[] PROGMEM   = { "//action: T%d" };
const char P_ActionWait[] PROGMEM   = { "//action: WAIT" };
const char P_ActionCont[] PROGMEM   = { "//action: CONTINUE" };
const char P_ActionPong[] PROGMEM   = { "//action: PONG" };
...
const char P_MCmds[] PROGMEM = { 
  ...
  "M569\t-\tSet TMC SpreadCycle on/off\n" \
  "M575\t-\tSet serial port baudrate\n" \
  "M700\t-\tLoad filament\n" \
  "M701\t-\tUnload filament\n" \
  "M906\t-\tSet TMC motor current\n" \
  "M914\t-\tSet TMC stall sensitivity\n" \
  "M999\t-\tReset\n" \
